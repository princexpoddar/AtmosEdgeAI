...
def engineer_features(df: pd.DataFrame, drop_na: bool = True) -> pd.DataFrame:
    ...
    # 1. Calendar/Temporal Features (Sine/Cosine encodings + Raw metrics)
    hours = df.index.hour
    ...
    if drop_na:
        # Save number of rows before drop
        before = len(df)
...
def get_temporal_feature_names() -> List[str]:
    """
    Returns list of temporal features used by the global forecasting model.
    """
    return [
